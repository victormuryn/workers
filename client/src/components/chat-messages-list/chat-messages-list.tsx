    ...
    >
      {
        messages.map((message, i, messages) => {
          ...
          const anotherDay = !datesAreOnSameDay(msgDate, new Date(prev));
          const longTime = !anotherDay && (+new Date(next?.date) - +msgDate > hour);

          const showAvatar = next?.from !== from || longTime;
            ...
            <>
              {
                anotherDay &&
                  ...
                  >{msg.date.toLocaleDateString()}</li>
              }

              <ChatMessage key={_id} message={msg} />

              {longTime && <li className="mb-3" />}
            </>
          );
        })
      }
